...
| **Note editing** (piano roll) | ✅ free-timed `BeatNote` | ✅ `beat add-note`/MCP | ✅ add/move/resize/multi-select/marquee · 🚧 pitch reference (Stream U) | Editing logic real since Phase 13/17; visual pitch anchor (keyboard strip/gridlines) landing now |
...
| **Render / export** | ✅ real engine, self-contained since Phase 17 (no BeatLab dependency) | ✅ `beat render` | 🚧 Stream X | No way to get a WAV out of the app without CLI until tonight |
