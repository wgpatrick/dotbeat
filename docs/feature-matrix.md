...
| **Synth sound design** (osc/filter/env/inserts/sends) | ✅ ~54-field `SYNTH_FIELDS` | ✅ `beat set` | ✅ full grouped panel (Phase 13) | Osc2 investigated (Stream Y): **no bug found** — apply chain confirmed correct on a controlled patch (identical result via GUI vs. direct doc edit); the original "barely changed" report was a measurement artifact (wrong probe on a patch where osc2 was near-unison with osc1, so raising its level added loudness not brightness) |
...
| **Automation** | ✅ `BeatAutomationLane` (v0.9), playback works | ✅ `beat automate` | ✅ per-track picker + draggable curve, playback verified to follow the drawn curve exactly | Phase 20 Stream Z. Deferred: same-row overlay (vs. dedicated sub-lane), curved segments, multi-clip-per-track, log y-axis |
