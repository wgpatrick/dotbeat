...
```bash
npm install
npm test                          # 634 tests: format, conversion, daemon sync, CLI, vary/humanize, DSP metrics, MCP
...
| Path | What |
|---|---|
| [`docs/product-roadmap.md`](docs/product-roadmap.md) | **Start here for what's built.** Every tracked feature (305 and counting), each rated done/in-progress/not-started across the core format, CLI/MCP, and GUI layers — the live source of truth, not a snapshot. |
...
| `test/` | 634 tests — format round-trips, conversion fidelity, daemon sync, CLI, DSP metrics vs known-answer signals, MCP protocol, vary/humanize/groove determinism. |
...
app it started from), a 634-test suite, a session-local undo/redo stack alongside the git-backed
...
proposal awaiting an owner decision in `docs/multi-region-audio-design.md`. `docs/product-roadmap.md`
