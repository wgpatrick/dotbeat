...
const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Add an insert to a synth track's effect chain (format v0.10). Array order in the file IS chain order — no separate index field — so this is dotbeat's answer to \"reorder/add/remove effects,\" built as flat ordered text rather than a box/pointer graph (docs/research/21-opendaw-devices-effects.md #1). type is one of eq3|comp|distortion|bitcrush|autoFilter|autoPan|tremolo|utility|grainDelay|vinylDistortion|resonator — the first four are the original built-in inserts every synth track already has knobs for (see beat_set's eqLow/compMix/distortionMix/bitcrushMix etc.); Phase 23 Stream BE added Auto Filter/Auto Pan/Tremolo/Utility (autoFilter*/autoPan*/tremolo*/utility* params — Redux's downsampling half rides bitcrushRate on the existing bitcrush type, not a new one); Phase 23 Stream BF added three real custom-DSP inserts — a hand-built granular delay, WaveShaper+seeded-noise vinyl character, and a tuned bandpass-filter-bank resonator (see beat_set's grainDelay*/vinyl*/resonator* fields for their own params, docs/phase-23-stream-bf.md for design notes). This tool only changes whether/where/in-what-order a type runs in the chain, not its own params. Omit id to mint one (the type name, or type_2/_3... on collision); omit index to append. Returns the musical edit list.",
    inputSchema: {
      type: 'object',
      properties: {
        file: { type: 'string' },
        track: { type: 'string' },
        type: { type: 'string', description: 'eq3 | comp | distortion | bitcrush | autoFilter | autoPan | tremolo | utility | grainDelay | vinylDistortion | resonator' },
