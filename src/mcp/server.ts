  ...
  // ==== Phase 39 Stream UB begin ====
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        backend: { type: 'string', description: '"stableaudio" (default, local model), "fal" (hosted API — same Stable Audio Open model on fal.ai GPUs, seconds instead of minutes; needs FAL_KEY server-side), or "stub" (deterministic dependency-free tone bed)' },
        provider: { type: 'string', description: 'provider label recorded in provenance (default "stable-audio-open"); for backend "fal" a fal model path selects the hosted model (default "fal-ai/stable-audio"; "fal-ai/stable-audio-25/text-to-audio" opts into Stable Audio 2.5 under Stability platform terms)' },
