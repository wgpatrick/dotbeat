...
const ANALYZE_PY = 'python/analyze.py' // relative to the repo root (kept relative so the sidecar's
// own `pip install -r python/…` fix lines are meaningful); spawned with cwd=repoRoot.

// The spawn scaffold — one interpreter rule, one timeout, one exit-code vocabulary for all eight
// sidecars — lives in ./spawn-sidecar.ts. resolvePython is RE-EXPORTED here because this module has
// been its home since Phase 38 and half the fleet imports it from './sidecar.js'.
export { resolvePython } from './spawn-sidecar.js'
...
export async function runAnalysis(opts: RunAnalysisOptions): Promise<RunAnalysisResult> {
  ...
  const res = await spawnSidecar({ python, args: [ANALYZE_PY, '--backend', backend, '--input', abs] })
...
export async function sidecarDoctor(): Promise<Record<string, unknown>> {
  return runSidecarDoctor(ANALYZE_PY, { hint: VENV_SETUP_HINT })
}
