...
`python/surge_render.py` no longer reads the broken 2-row `getOutput()`. It processes one block at a
time (so note-on/off events still land at their exact sample-quantized positions) but collects each
block through `processMultiBlock` into a `createMultiBlock(1)` buffer, whose C++ `memcpy` path
delivers the true left and right channels. The `ringDb` screen stays in place as a safety net
regardless.
