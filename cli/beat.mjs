...
const HELP = [
  ...
  },
  {
    ...
    text: `  beat rolecheck <file.wav> --role <bassline|chords|lead|drum-loop> [--json] [--targets <file.json>]
                                                          PRE-BATCH pass/fail against the per-role audio targets
                                                          mined from the owner's own packs-era Splice reference
                                                          clips (presets/role-targets.json, regenerated by
                                                          scripts/build-role-targets.mjs). Prints measured / target
                                                          / verdict per axis and, for every MISS, the specific
                                                          dotbeat parameter to change and the research doc that
                                                          says so. Exit 1 on FAIL so a batch script can gate on it.
                                                          Research 138's design rule: every arm's clips pass their
                                                          feature gates BEFORE entering a batch — never spend owner
                                                          ratings on a clip that missed its own targets.
                                                          NOT a quality verdict: thresholds are percentiles of the
                                                          reference pool, so ~a quarter of the owner's own winning
                                                          refs would miss any given check, and no feature here
                                                          hears harmony or groove (131 §8). Roughness is absent by
                                                          design — it has no valid ABSOLUTE threshold (123, 131
                                                          §4); use beat lint --roughness-baseline for the
                                                          pair-relative grind screen.`,
  },
  {
    cmd: 'lint',
