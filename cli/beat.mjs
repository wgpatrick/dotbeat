...
const USAGE = `usage:
  ...
  beat effect-add <file> <track> <eq3|comp|distortion|bitcrush|autoFilter|autoPan|tremolo|utility|grainDelay|vinylDistortion|resonator> [--id id] [--index n] [--bypassed]
...
function effectAddCmd(argv) {
  ...
  if (!file || !track || !type) throw new BeatEditError('effect-add needs <file> <track> <eq3|comp|distortion|bitcrush|autoFilter|autoPan|tremolo|utility|grainDelay|vinylDistortion|resonator> [--id id] [--index n] [--bypassed]')
  const before = readDoc(file)
