...
class Engine {
  ...
  private tick(time: number): void {
    ...
    for (const track of doc.tracks) {
      ...
      // the live audio graph. ('off'/'pitch'/'cutoff' are handled above; 'wtPos' is a deliberate
      // no-op — wavetable oscillators aren't ported, see the file-header scope note.) Every case
      ...
      // discarding it (Bug 2). This also folds 'amp' (volume) into the same rule instead of its
      // previous special-cased, opposite-order handling (automation used to always beat the LFO
      // for volume specifically) — now all three cases (cutoff, volume, everything else) share one
      // consistent compose-with-automation rule.
