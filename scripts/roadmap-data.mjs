...
export const rows = [
  ...
  },
  {
    ...
    description: 'Today, appended sections share the source scene; editing one edits them all. Ableton solves this with two commands: Insert Scene (a blank, independent, empty scene inserted at a position) and Capture-and-Insert Scene (snapshot whatever\'s currently live into a brand-new scene, no audible interruption) — manual ch.7 p.182. Both building blocks already exist in dotbeat: `songAppend`/`setSong` offer only whole-list replace and reorder (no "insert a new independent BeatScene at a position" primitive yet), and `sceneFromLiveContent` (src/daemon/daemon.ts:200) already does the capture half but is invoked automatically exactly once, at loop→song conversion — this is wiring it into a repeatable, user-triggered GUI action, not new capability. New `BeatScene` splice primitive (empty slots) for Insert Scene; generalize `sceneFromLiveContent` for Capture-and-Insert. Bundle as one stream — they share the "new section at a position" plumbing.',
    core: 'missing', cli: 'missing', gui: 'missing', status: 'not-started',
  ...
  // ── Macros ───────────────────────────────────────────────────────────────
  {
    ...
    description: 'A curated "front panel" of knobs mapped to real params, living outside the file (like presets) — turning a macro writes literal edits, never an in-file indirection. Phase 23 research stream RC scoped the concrete data shape (BeatMacro/MacroTarget), storage (presets/macros.json via the existing /library route), and GUI placement (a Macros row in SynthPanel.tsx) — see research/27. Two independent chapter comparisons (63, 64) each call this "the single biggest scoped-but-unbuilt gap" surfaced by their whole comparison against Ableton\'s Macro Controls (up to 16 mappable knobs, 8 visible by default, min/max/curve per mapping — manual ch.24 pp.474-475). Build directly from research 27\'s design: `src/core/macro.ts` (`BeatMacro`/`MacroTarget`/`resolveMacro`/`applyMacro`, mirrors `src/core/preset.ts`), `presets/macros.json` (8-macro starter set), `GET /library` gains a `macros` array + `POST /library/apply-macro`, CLI `beat macro list/apply` + MCP tools. Resolves to literal edits, no in-file indirection (D9) — the single largest breach-free capability gap in the whole synth/drum sound-design comparison.',
    core: 'missing', cli: 'missing', gui: 'missing', status: 'not-started',
