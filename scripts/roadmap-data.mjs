...
export const rows = [
  ...
  },
  {
    ...
    description: 'Phase 26 Stream DI shipped this: `BeatAutomationPoint.interpolation?: \'linear\' | \'hold\' | \'curve\'` (document.ts), canonically elided when \'linear\' (a trailing `interpolation=hold|curve` token, same discipline as v0.10\'s note fields — `pointOptionalTokens`/`parsePointOptionalFields` in serialize.ts/parse.ts). Lives on the segment\'s START point, mirroring Ableton\'s own per-segment gesture. GUI (`AutomationLane`, ArrangementView.tsx): Alt/Option-drag a segment bows it (a live quadratic-bezier-toward-the-drag-point preview, committing a `curve` flag on release — the committed render/engine both use a fixed quadratic ease-in, `curveEase`); a per-point right-click popup offers a linear/hold/curve toggle AND an exact numeric `<input>` (both write through the same `set` op); the live drag-value label (`drag.value`, previously computed but never rendered) now shows in a floating tag while dragging. Engine (`interpolateAutomation`, engine.ts): branches on the flag — \'hold\' steps instantly to the next point\'s value at its time instead of ramping, \'curve\' eases via `curveEase` before the linear/log blend — a real playback difference, verified with actual recorded/measured audio (not just internal state) in `ui/verify-phase26-stream-di.mjs`. CLI/MCP: `beat automate --interpolation linear|hold|curve` / `beat_automate`\'s `interpolation` arg.',
    core: 'done', cli: 'done', gui: 'done', status: 'done',
    research: 'research/65-ableton-vs-dotbeat-automation-envelopes.md', plan: 'phase-26-plan.md',
  },
  {
  ...
  {
    area: 'Machine listening, composition & gen providers (taste program)', feature: 'Compose a figure into a real project (`beat compose` / `beat_compose`)',
    description: 'The VERB that was missing between dotbeat’s two composition engines and an ordinary project. Both engines already worked — the theory layer (theory.ts) and Composer’s Assistant 2 (ca2.ts) — but both were reachable ONLY from `beat showdown --theory`/`--ca2`, whose job is blind engine comparison, not authoring. Found the hard way on 2026-07-27: producing a real 9-section song the owner needed arp figures and had to hand-roll rule-based cells in a throwaway Python script, because no command could say "compose a figure into THIS track of THIS project". `beat compose <file> <track> [--source theory|ca2] [--role] [--seed] [--archetype] [--bars] [--key/--mode] [--register] [--append] [--clip] [--no-clip-sync] [--dry-run]` composes into the project and prints the musical edit list; `--count N [--out-dir]` writes an option board instead — N genuinely different figures (the archetype bank swept one variant at a time, identical figures rejected and re-seeded) as v1.beat..vN.beat + the same manifest.json `beat render --batch`, `beat board` and `beat adopt` already read. Three problems the showdown never had to solve, because it always composed into a fresh 4-bar seed, are owned here: KEY (resolved explicit > the track’s declared scale > inferSeedKey’s pitch-class histogram, with the winning source always printed — a guessed key is never silent); REGISTER (whole-octave match onto the target track’s own median pitch, since a lead composed at theory.ts’s own register lands an octave below a track already written up); and CLIPS — in song mode the engine renders a track’s CLIPS, never its live notes (engine.ts contentOf), so composing notes without re-snapshotting them produces variants that render BYTE-IDENTICAL to the parent. That is not hypothetical: it cost two full board renders on 2026-07-27 and was caught only because all eight variants measured identically. Every clip the track is placed under is now re-snapshotted by default (narrowable with --clip, opt-out-able with --no-clip-sync, which warns loudly). Shared logic in ONE module (src/compose/compose.ts) that the CLI and beat_compose both import, with three CLI/MCP parity rows. That module path is itself a finding: beat_compose first landed importing src/taste and was caught by test/import-boundary.test.ts (D8 / research/136 section 4 — the DAW, src/mcp included, may not import the taste program at runtime). The fix was the remedy 136 itself prescribes rather than a dodge: the pitched-composition layer (phrase/motif/theory/ca2, plus applyComposedPhrase and inferSeedKey lifted out of showdown.ts) moved to a new peer module src/compose/, and src/taste imports it back. None of it ranks anything — it is generators and document vocabulary that had simply been filed with the eval harness — so an MCP process now loads the composer without loading the ranker, the WAV codec or the reporting layer. CA2 stays optional-by-environment: --source ca2 fails with the doctor pointer where CA2 is not installed, never a substituted theory figure. 25 tests (test/compose-verb.test.ts), every "did this change what you hear" assertion resolved through a re-implementation of the engine’s own clip resolution rather than through the file’s bytes, and the clip trap pinned by its own negative control (the same compose with clip-sync off must come out clip-identical to the parent); proven to fail on the unfixed behavior.',
    core: 'done', cli: 'done', gui: 'na', status: 'done',
    research: 'research/124-midi-composition.md', plan: null,
  },
  {
  ...
  {
    area: 'Research 130-139 backlog', feature: 'The role-true width map, in the SHOWDOWN profile (140 D9)',
  ...
  {
    area: 'Research 130-139 backlog', feature: 'The MOTION half of produced defaults (140 D27)',
