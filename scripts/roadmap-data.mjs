...
export const rows = [
  ...
  {
    area: 'Render / export', feature: '`beat feedback` has no `--offline`, so the arrangement gate cannot run unattended',
    description: 'FOUND 2026-07-27 producing songs/twin-souls-study. `beat render` grew `--offline` (D22/D23) and it is exact, reproducible and survives anything; `beat feedback` still re-renders through REAL-TIME CAPTURE only, so verifying a full-length song costs its wall-clock duration with a live headless browser held open the whole time. On a 5:22 song that is 5:22 of fragility: two of three attempts died with `Target page, context or browser has been closed` when the machine slept, and the third only succeeded because the machine stayed awake. This matters more than an ordinary speed complaint because `feedback --sections --ref` is THE gate that checks arrangement against a measured reference arc — the one thing no static whole-file lint can see, and the check that caught two real section-contrast failures on this song (break-1 +3.8 LU, break-3 -7.8 LU vs the reference arc) that every other screen passed. It is also the gate most likely to be run on a long song, i.e. exactly where real-time capture is least affordable. Fix is plumbing, not design: `feedbackCmd` (cli/beat.mjs) calls `renderToBuffer`, which already takes the offline path — thread a `--offline` flag through, and consider defaulting it for songs over ~2 minutes. TRIGGER: none needed; the next full-song produce run hits this again.',
    core: 'partial', cli: 'missing', gui: 'na', status: 'not-started',
    research: null, plan: null,
  },
  {
