...
export const rows = [
  ...
  // answer, and the same reasoning puts errands here. Each row below needs a yes/no from the owner,
  // not a build slot.
  {
    area: 'Owner errands', feature: 'Email the m2m authors for a licence',
    description: 'Research 126 is the ONLY doc in its range with no roadmap row and no repo footprint whatsoever (`rg -i "m2m|music2music"` returns zero hits outside the doc). It measured two Mac-feasible capabilities worth having — `m2m_drummer` at note-F1 79.3 against CA2\'s 20.3, and `m2m_arranger` — and the models are unlicensed, i.e. all rights reserved by default, so nothing could be built. Nobody emailed the authors, and nobody wrote down that somebody should. The doc\'s own careful framing ("establishes technical feasibility and musical fit, not permission") made it easy to file as "done, blocked" rather than "one email away". Cost: one email.',
    core: 'na', cli: 'na', gui: 'na', status: 'not-started',
    research: 'research/126-music2music-models.md', plan: null,
  },
  {
    area: 'Owner errands', feature: 'File (or explicitly decline to file) the upstream surgepy issue',
    description: 'docs/research/surge-right-ear-ring-rootcause.md:99 still reads "Draft issue text for the..." and no issue or PR number appears anywhere in the repo; D23\'s own wording calls it an "upstream issue DRAFT", so the word "draft" is load-bearing. Research/140 reads the delay as CORRECT CAUTION rather than neglect — filing an upstream GitHub issue is an outward-facing action and no agent should do that unilaterally — and locates the failure precisely: nobody asked. It is also the only outward-facing contribution this project has ready to give; the body, a six-line repro and a one-line fix are all already written. Either file it or edit D23 to say "declined to file". (The internal half shipped correctly: surge_render.py now collects via processMultiBlock, never getOutput().)',
    core: 'na', cli: 'na', gui: 'na', status: 'not-started',
    research: 'research/136-architecture-review.md', plan: 'docs/decisions.md',
  },
  {
    area: 'Owner errands', feature: 'Rights reads: MiniMax ToS, ElevenLabs rights table, Splice ToU on matched patches',
    description: 'Three separate ten-minute reads, each blocking a build. Research 127 §4.5 needs the MiniMax terms-of-service read and the ElevenLabs rights table filled in before either adapter\'s output can enter a rated batch (both are currently tagged training-excluded by default, which is the safe assumption, not a verified one). Research 134 §4.3 needs a Splice-ToU ruling on whether patches MATCHED to purchased loops inherit any restriction — which gates the whole match-to-owned direction. Also here: the Surge factory/third-party licensing re-verification (#6741), which D23 itself lists as an open "revisit when".',
    core: 'na', cli: 'na', gui: 'na', status: 'not-started',
    research: 'research/127-gen-backend-bakeoff.md', plan: null,
  },
  {
